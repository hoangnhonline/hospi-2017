		...
		function offer_details($offerid = null) {
            ...
            $relatedHotels = $this->ci->hotels_lib->getRelatedHotels($rhotels);
                ...
                $detailResults = (object)array(
                	'id' => $details[0]->offer_id,'title' => $title, 'slug' => $slug,'thumbnail' => $thumbnail,'location' => $location->city,
            'desc' => $desc,'sliderImages' => $sliderImages,'price' => $offerPrice,'currCode' => $curr->code,'relatedItems' => $relatedHotels,
...
//offer short details
	...
	// get Offer images
		...
		function showOffers($offset = null,$loc = null) {
			$this->ci->load->library('bootpagination');
				...
				$rh = $this->ci->special_offers_model->list_specialOffers_front('','','',$loc);
			    $offers = $this->ci->special_offers_model->list_specialOffers_front($perpage, $offset, $orderby,$loc);
		...
		//make a result object all data of offers array
         function getResultObject($offers){
          ...
          foreach($offers as $o){
            ...
            'offerForever' => $offerForever,'fullExpiryDate' => $fullExpiry, 'city' => $city->city,'cityid' => $cityid, 'offers_created_at' => $o->offers_created_at);
            }
